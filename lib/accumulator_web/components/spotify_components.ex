defmodule AccumulatorWeb.SpotifyComponents do
  ...
  def now_playing(assigns) do
    ...
    <a
      ...
      target="_blank"
      rel="noreferrer"
      class="flex gap-4 items-center p-4 rounded-md hover:bg-opacity-30 hover:bg-black"
    >
      <div class="w-max">
      ...
      <div>
        ...
        <p class="text-sm text-gray-400 my-2"><%= @track["artists"] %></p>
  ...
  def refresh_now_playing(assigns) do
    ~H"""
    <button phx-click="refresh-now-playing" class="mt-2 text-sm text-blue-500 hover:text-blue-400">
      Refresh
    </button>
    """
  end
