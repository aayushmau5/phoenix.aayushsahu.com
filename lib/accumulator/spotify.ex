defmodule Accumulator.Spotify do
  ...
  @spec get_now_playing() ::
          {:ok, Helpers.t()} | {:not_playing, String.t()} | {:error, String.t()}
  ...
  """
  @spec get_top_tracks() :: {:ok, [Helpers.t()]} | {:error, String.t() | any()}
  ...
  defp get_and_cache_now_playing do
    with {:ok, access_token} <- get_access_token() do
      response = API.now_playing(access_token)
  ...
  defp get_and_cache_top_tracks do
    with {:ok, access_token} <- get_access_token() do
      response = API.top_tracks(access_token)
